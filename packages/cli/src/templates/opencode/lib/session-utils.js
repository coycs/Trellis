...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (taskStatus === "planning") {
    ...
    if (missingComplex.length > 0) {
      ...
      nextBits.push(
        "Planning artifacts are present; ask for review before `trellis task start`",
      );
    }
    if (!jsonlReady) {
      nextBits.push(
...
function buildCompactCurrentState(ctx, platformInput, specIndexPaths) {
  ...
  if (existsSync(tasksDir)) {
    try {
      ...
      lines.push(
        `Active tasks: ${activeTasks.length} total. Use \`trellis task list --mine\` only if needed.`,
      );
    } catch {
...
export function buildSessionContext(ctx, platformInput = null) {
  const directory = ctx.directory;
  const paths = collectSpecIndexPaths(directory, null);
...
Trellis compact SessionStart context. Use it to orient the session; load details on demand.
  ...
  if (workflowContent) {
    ...
    const overviewLines = [
      ...
      "Full guide: .trellis/workflow.md. Step detail: `trellis context --mode phase --step <X.Y>`.",
      "",
    ];
  ...
  parts.push("Discover more via: " + "`trellis context --mode packages`");
  parts.push("</guidelines>");
